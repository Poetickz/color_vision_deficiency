...
for c,d in zip(colors, data_set['intensity']):
    if d > 0:
        new_colors.append((brigthness_level(c[0],d,level), brigthness_level(c[1],d,level), brigthness_level(c[2],d,level)))
    else:
        new_colors.append((c[0]*level, c[1]*level, c[2]*level))
