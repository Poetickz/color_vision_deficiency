...
for c in colors:
    new_colors.append(colorsys.rgb_to_hls(c[0],c[1],c[2]))

colored = []
for c,d in zip(new_colors, data_set['intensity']):
    if d > 0:
        colored.append((c[0], brigthness_level(c[1],d,level), c[2]))
    else:
        colored.append((c[0], c[1]*level, c[2]))


new_colors = []

for c in colored:
    new_colors.append(colorsys.hls_to_rgb(c[0], c[1], c[2]))
